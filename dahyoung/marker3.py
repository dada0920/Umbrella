...
class Marker3:
    ...
    def SearchPharmacy(self,addr):
        ...
        lat = "37.6803112" # 검색할 기준 경도 입력
        lng = "127.0549036" # 검색할 기준 위도 입력
        Hdistance = "500" # 검색할 반경 입력
        addr = "?lat="+lat+"&lng="+lng
        distance = "&m="+Hdistance
        # address = urllib.parse.quote(addr)
        url_address = urljoin(url,addr,distance)
    ...
    def main(self):
        ...
        if(jres == None):
            print("json.loads 실패!!!")
            exit()
        Pharmacy =  "우리조은약국" # 검색할 약국 이름을 입력
        yg_info = self.searchYG(jres["stores"], Pharmacy)
    ...
    def main2(self):
        ...
        stock = 'empty' #  수량을 입력
        mask_info = self.searchMK(jres["stores"], stock)
